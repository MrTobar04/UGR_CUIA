...
class escenaPYGFX:
    def __init__(self, fov, ancho, alto):
        ...
        global _global_renderer, _global_canvas
        with _renderer_lock:
            if _global_renderer is None:
                _global_canvas = WgpuCanvas(size=(ancho, alto))
                _global_renderer = gfx.WgpuRenderer(_global_canvas)
            # Reutilizamos la instancia global.
            self.canvas = _global_canvas
            self.renderer = _global_renderer
    ...
    def eliminar_modelo(self):
        self.scene.clear()
        gc.collect()  # Forzar liberación de memoria si es necesario
